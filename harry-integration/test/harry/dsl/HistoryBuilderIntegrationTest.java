...
import harry.model.ModelTestBase;
...
public class HistoryBuilderIntegrationTest extends ModelTestBase
{
    ...
    public void simpleDSLTest() throws Throwable
    {
        ...
        for (int i = 0; i < SchemaGenerators.DEFAULT_RUNS; i++)
        {
            ...
            modelChecker.init(new HistoryBuilder(seed, maxPartitionSize, 10, schema))
                        .step((history) -> {
                        ...
                        .exitCondition((history) -> {
                            ...
                            ReplayingVisitor visitor = historyBuilder.visitor(run.tracker, run.sut);
                            visitor.replayAll();

                            if (historyBuilder.presetSelector.pds().size() < MAX_PARTITIONS)
                                return false;

                            Model model = historyBuilder.checker(run.tracker, sut);
